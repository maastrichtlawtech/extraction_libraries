    ...
    if dataframe is None and filename is None and save_file == 'y':
        ...
        if len(csv_files) > 0 and save_file == 'y':
            for f in csv_files:
                ...
                logging.info(f"Available columns in the DataFrame: {rs_data.columns.tolist()}")
                if 'id' not in rs_data.columns:
                    print(f"it is not empty but has columns {rs_data.columns.tolist()}")
                    logging.error("'id' column is missing in the DataFrame. Please check the input data.")
                    return False
